...
const FinancePage: React.FC = () => {
  ...
  const { data: expenses, isLoading: loadingExpenses } = useQuery({ queryKey: ['expenses'], queryFn: financeService.getAllExpenses });
  const { data: stats } = useQuery({ queryKey: ['financeStats'], queryFn: financeService.getStats });
  ...
  return (
    <div>
      ...
      <div className="bg-white shadow-sm border border-gray-200 rounded-2xl overflow-hidden min-h-[600px] flex flex-col">
        <div className="border-b border-gray-100 px-6 pt-2 bg-gray-50/30">
            ...
            {['invoices', 'expenses', 'tax'].map((tab) => (
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors capitalize ${
                  activeTab === tab ? 'border-sey-blue text-sey-blue' : 'border-transparent text-gray-500 hover:text-gray-700'
                }`}
              >
                {tab}
              </button>
